...
- `If` / `While` / `For` 条件已缓存 `getline > 0` 快路径目标（`AwkGetlineTarget?`），执行期不再反复从条件 AST 做字符串化识别
...
1. 统一 `Getline < file` 的长期策略（当前函数态为显式不支持；后续可评估是否引入受控 FS 上下文）。
2. 迁移完成后清理剩余中间兼容辅助函数与重复逻辑。
